...
在学习研究BM算法之前，最好已经掌握KMP算法，所以建议还没有掌握的同学，先去学习一下，循序渐进的来，可以看[KMP算法]({{ site.url }}/posts/string-kmp-algorithm/)。为什么说循序渐进，是因为BM算法在大多数情况下表现的比KMP算法优秀，所以大部分时候，都当做KMP进阶的算法来学习。BM算法从模式串的尾部开始匹配，且拥有在最坏情况下 `O(N)` 的时间复杂度。有数据表明，在实践中，比 KMP 算法的实际效能高，可以快大概 3-5 倍，很值得学习。
...
![BCandGS]({{ site.url }}/assets/img/2024-12-25-string-bm-algorithm/BCandGS.png)
